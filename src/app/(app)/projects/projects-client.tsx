...
export default function ProjectsClient({ initialTasks, projects, agents, customers, artifacts }: any) {
    ...
    return (
        <div className="h-full flex flex-col space-y-6 animate-in fade-in duration-500 relative">
            ...
            {selectedTask && (
                <div className="absolute top-0 right-0 w-1/3 h-full bg-zinc-900/95 border-l border-zinc-800 shadow-2xl backdrop-blur-3xl animate-in slide-in-from-right-8 duration-300 p-6 flex flex-col z-50 rounded-xl">
                    ...
                    <div className="pt-6 border-t border-zinc-800 flex flex-col space-y-3 mt-auto">
                        <div className="text-xs text-zinc-500 font-medium">Send Context / Notes to OpenClaw:</div>
...
function TimelineEvent({ time, desc }: { time: string, desc: string }) {
    return (
        <div className="relative pl-4">
            <div className="absolute -left-[5px] top-1.5 w-2 h-2 rounded-full bg-zinc-700 ring-4 ring-zinc-900" />
            <div className="text-xs font-mono text-zinc-500 mb-0.5">{time}</div>
