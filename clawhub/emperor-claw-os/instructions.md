...
- Use customer-scoped artifacts by default. Only attach `projectId` when the document belongs to project work, and only attach `taskId` when that project document is tied to a specific task.
