...
function resolveOpenClawCliJsPath(cliPath) {
...
function readConfiguredAgentIds(configPath) {
...
async function ensureRuntimeDeps(runtimeDir) {
...
async function runControlPlaneBootstrap(runtimeDir, companionDir, stateDir, bridgeStatePath, input, runtimeId) {
...
async function resolveEmperorAgentId(apiUrl, token, agentName) {
    try {
        ...
        const match = rows.find((row) => String(row?.name || "").trim() === String(agentName || "").trim());
        return match?.id;
    }
    catch {
        return undefined;
...
function readBridgeStateAgentId(bridgeStatePath) {
...
async function waitForEmperorAgentId(apiUrl, token, agentName, bridgeStatePath) {
    const deadline = Date.now() + 15000;
    while (Date.now() < deadline) {
        const fromBridgeState = readBridgeStateAgentId(bridgeStatePath);
        if (fromBridgeState)
            return fromBridgeState;
...
async function emperorFetch(apiUrl, token, pathname, init = {}) {
...
async function upsertCompanySharedDoctrineResources(apiUrl, token) {
    const createdIds = [];
    for (const spec of getSharedDoctrineResourceSpecs()) {
        ...
        const existing = resources.find((resource) => String(resource?.scopeType || "").trim() === "company"
            && String(resource?.name || "").trim() === spec.name) || null;
        const body = {
            name: spec.name,
...
async function reconcileEmperorAgentProfile(apiUrl, token, agentId, profile) {
...
async function ensureLocalBrainAgent(localBrainAgentId, workspaceDir, agentName, openclawConfigPath) {
...
function writeEnvFile(envFile, values) {
    const lines = Object.entries(values).map(([key, value]) => `${key}=${JSON.stringify(value)}`);
    fs.writeFileSync(envFile, `${lines.join("\n")}\n`, "utf8");
}
function writeBridgeConfig(companionDir, values) {
    ...
    const config = {
        ...
        logLevel: values.EMPEROR_CLAW_LOG_LEVEL,
        logFormat: values.EMPEROR_CLAW_LOG_FORMAT,
        stateDir: values.EMPEROR_CLAW_STATE_DIR,
        bridgeStatePath: values.EMPEROR_CLAW_BRIDGE_STATE_PATH
    };
    fs.writeFileSync(configPath, `${JSON.stringify(config, null, 2)}\n`, "utf8");
}
function writeRunBridge(companionDir) {
    if (process.platform === "win32") {
...
function writeSystemdService(serviceNameBase, envFile, companionDir) {
    if (process.platform === "win32")
        return;
...
async function downloadControlPlane(runtimeDir, apiUrl) {
...
async function copyRuntimeAssets(pluginRoot, runtimeDir, companionDir, apiUrl) {
...
export async function bootstrapAgent(paths, input) {
    if (!input.token?.trim())
        throw new Error("Emperor token is required");
    if (!input.agentName?.trim())
        throw new Error("Agent name is required");
    if (!input.localBrainAgentId?.trim())
        throw new Error("Local brain agent id is required");
    ...
    const envValues = {
        ...
        EMPEROR_CLAW_LOG_LEVEL: "info",
        EMPEROR_CLAW_LOG_FORMAT: "jsonl",
        EMPEROR_CLAW_LOG_PROMPTS: "false",
