...
async function fetchThreads(token, apiUrl) {
    ...
    if (!response.ok)
        throw new Error(`thread fetch failed with ${response.status}`);
    const payload = await response.json();
    return payload.threads || [];
}
async function fetchThreadMessages(token, apiUrl, threadId) {
    const response = await fetch(`${apiUrl.replace(/\/+$/, "")}/api/mcp/threads/${threadId}/messages?limit=200`, {
        method: "GET",
        headers: { Authorization: `Bearer ${token}` }
    });
    if (!response.ok)
        throw new Error(`thread message fetch failed with ${response.status}`);
    const payload = await response.json();
    return payload.messages || [];
}
export function registerEmperorCli(api, program) {
    ...
    emperor.command("show-agent")
        ...
        if (!manifest)
            return print(`No tracked Emperor agent found for ${opts.localBrainAgentId}`);
        print([
    ...
    emperor.command("add-agent")
        ...
        const result = await bootstrapAgent(paths, {
            ...
            profile: String(opts.profile || "operator"),
    ...
    emperor.command("repair").description("Repair and restart tracked Emperor bridge agents").action(async () => {
        ...
        print(repaired.length === 0 && upgraded.changed === 0
    ...
    emperor.command("remove-agent")
        ...
        if (!result)
            return print(`No tracked Emperor agent found for ${localBrainAgentId}`);
        print([
    ...
    emperor.command("rebind-threads")
        ...
        for (const thread of threads) {
            ...
            const targetedHuman = messages.find((message) => message?.senderType === "human" && message?.targetAgentId);
            if (targetedHuman?.targetAgentId) {
                owners[thread.id] = targetedHuman.targetAgentId;
                rebound += 1;
                continue;
            }
            const firstAgent = messages.find((message) => message?.senderType === "agent" && (!knownAgentIds.size || knownAgentIds.has(message.senderId)));
