...
export type RemoveTrackedAgentInput = {
  localBrainAgentId: string;
  removeCompanionDir?: boolean;
  removeWorkspace?: boolean;
  removeLocalBrainAgent?: boolean;
};

export type RemoveTrackedAgentResult = {
  localBrainAgentId: string;
  manifestPath: string;
  threadBindingsRemoved: number;
  systemdUnitRemoved: boolean;
  companionRemoved: boolean;
  workspaceRemoved: boolean;
  localBrainAgentRemoved: boolean;
  localBrainAgentRemovalAttempted: boolean;
};
...
async function removeSystemdUnitFile(serviceName: string): Promise<boolean> {
...
async function removeLocalBrainAgent(agentId: string, openclawCliPath?: string): Promise<boolean> {
...
export async function removeTrackedAgent(paths: EmperorPluginPaths, input: RemoveTrackedAgentInput): Promise<RemoveTrackedAgentResult | null> {
