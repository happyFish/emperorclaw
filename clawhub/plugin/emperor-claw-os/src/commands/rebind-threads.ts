...
async function fetchThreads(token: string, apiUrl: string): Promise<any[]> {
  const { stdout } = await execFileAsync("curl", [
    ...
    `${apiUrl}/api/mcp/threads?thread_type=direct`
  ]);
...
async function fetchThreadMessages(token: string, apiUrl: string, threadId: string): Promise<any[]> {
  const { stdout } = await execFileAsync("curl", [
    "-sS",
    "-H", `Authorization: Bearer ${token}`,
    `${apiUrl}/api/mcp/threads/${threadId}/messages?limit=200`
  ]);
  const payload = JSON.parse(stdout);
  ...
  api.registerCommand({
    ...
    async execute(_invocationId: string, params: any) {
      ...
      return {
        content: [{
          type: "text",
          text: `Rebound ${rebound} direct thread owner mappings into ${paths.threadOwnerPath}`
        }]
      };
    }
  });
